...
graph box total_duration, yline(`r(mean)', lpattern(.)) ///
	...
	note("Note:" "Total clients = `r(N)'" "Survey duration above 100 minutes is dropped" "Number of dropped observations = `dropped_obs'", size(medsmall))
...
**# Summary table
labvars q_1a_1 q_1a_2 q_1a_4 q_1b_1 q_1b_2 q_2a_1 q_2a_2 q_2a_4 q_2b_1 q_2b_2 q_4 q_11b q_12d q_12f ///
"Amount of last cash deposit w/ BM Agent" ///
"Last charged cash deposit fee w/ BM Agent" ///
"Number of deposits w/ BM Agent last 3 months" ///
"Amount of last cash deposit w/ non-BM Agent" ///
"Number of deposits w/ non-BM Agent last 3 months" ///
///
"Amount of last cash withdrawal w/ BM Agent" ///
"Last charged cash withdrawal fee w/ BM Agent" ///
"Number of withdrawals w/ BM Agent last 3 months" ///
"Amount of last cash withdrawal w/ non-BM Agent" ///
"Number of withdrawals w/ non-BM Agent last 3 months" ///
///
"Level of satisfaction w/ BM Agent service" ///
"Number of agents in the area" ///
"Pc of overall agent transactions w/ BM Agent" ///
"Economic ladder position"
...
la def consent 1 "Compensation [N = `obs_consent_comp']" 2 "Lottery [N = `obs_consent_lot']"
...
graph bar gr_comp_*, percentages /// percent is the default
	...
	note("Note:" "Total clients = `r(N)'", size(medsmall))
...
histogram q_1a_1, percent color(brown) ///
	...
	subtitle(" ", size(medsmall)) ///
	note("Note:" "Total clients = `r(N)'" "Outlier threshold = `=string(`ll', "%15.0gc")' (lower) and `=string(`ul', "%15.0gc")' (upper)" "Dropped outlier observation = `dropped_obs'", size(medsmall))
...
graph box q_1a_1, yline(`r(mean)', lpattern(.)) ///
	...
	note("Note:" "Total clients = `r(N)'" "Outlier threshold = `=string(`ll', "%15.0gc")' (lower) and `=string(`ul', "%15.0gc")' (upper)" "Dropped outlier observation = `dropped_obs'" "The sumstat value is rounded to the nearest value of 1,000", size(medsmall))
...
graph box q_1a_2, yline(`r(mean)', lpattern(.)) ///
    ...
    text(`r(p25)' 95 "Q1=`=string(`Q_1_rounded', "%15.0gc")'", size(vsmall)) ///
    text(`r(mean)' 95 "Mean=`=string(`mean_rounded', "%15.0gc")'", size(vsmall)) ///
	note("Note:" "Total clients = `r(N)'" "Outlier threshold = `=string(`ll', "%15.0gc")' (lower) and `=string(`ul', "%15.0gc")' (upper)" "Dropped outlier observation = `dropped_obs'" "The sumstat value is rounded to the nearest value of 100", size(medsmall))
...
histogram q_1a_4, percent color(brown) ///
	...
	note("Note:" "Total clients = `r(N)'" "Outlier threshold = `ll' (lower) and `ul' (upper)" "Dropped outlier observation = `dropped_obs'", size(medsmall))
...
graph bar (sum) answer*, stack percentage over(q_5a, label(angle(30) labsize(vsmall))) ///
	...
	note("Note:" "Total clients = `obs5a'", size(medsmall)) ///
	ytitle("%", size(medsmall) orientation(horizontal)) ///
	ylabel(, labsize(medsmall)) ///
...
graph bar (sum) answer*, stack percentage over(q_7, label(angle(30) labsize(small))) ///
	...
	note("Note:" "Total clients = `obs7'", size(medsmall)) ///
	ytitle("%", size(medsmall) orientation(horizontal)) ///
	ylabel(, labsize(medsmall)) ///
	blabel(bar, pos(center) size(vsmall) format(%15.1fc))
...
graph bar (sum) answer*, stack percentage over(q_8, label(angle(30) labsize(small))) ///
	...
	note("Note:" "Total clients = `obs8'", size(medsmall)) ///
	ytitle("%", size(medsmall) orientation(horizontal)) ///
	ylabel(, labsize(medsmall)) ///
	blabel(bar, pos(center) size(vsmall) format(%15.1fc))
...
graph bar (sum) answer*, stack percentage over(q_9, label(angle(30) labsize(small))) ///
	...
	note("Note:" "Total clients = `obs9'", size(medsmall)) ///
	ytitle("%", size(medsmall) orientation(horizontal)) ///
	ylabel(, labsize(medsmall)) ///
	blabel(bar, pos(center) size(vsmall) format(%15.1fc))
