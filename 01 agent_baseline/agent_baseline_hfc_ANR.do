...
*IMPORT DATA*
*************
	use "$dta/cleaned_baseline_agent_survey_09012026", clear 	
...
preserve
	...
	***# DAILY N plot
    ...
    ytitle("Responses per day (N)") ///
    xtitle("Days since launch of group") ///
    title("Daily Responses by Treatment") ///
    ylabel(, grid) ///
    xline(1 3 7 14 21, lpattern(dash) lcolor(gs12))

	graph export "$output/Agent Baseline - `date'/1 - daily_response_plot.png", as(png) replace
	
	
	*** # CUMULATIVE N plot	
	*** JALANKAN SEMUA BARIS INI SEKALIGUS ***
	...
	* 2. Isi Macro (Gunakan variabel 'progress' yang sudah didestring)
	levelsof treatment_status, local(levels)
	foreach l in `levels' {
		local lbl : label (treatment_status) `l'
		if "`lbl'" == "" local lbl "Group `l'"
		local col : word `i' of blue red green orange purple pink gs8
    
    * Tambahkan baris ini ke macro
    local dayplots `dayplots' (line progress startdate if treatment_status == `l', lcolor(`col'))
    local legendorder `legendorder' `i' "`lbl'"
    local ++i
	}

	* 3. Gambar Grafik (Sekarang `dayplots' ada isinya)
	twoway `dayplots', ///
		legend(order(`legendorder') cols(3) pos(6) ring(1)) ///
		ytitle("Responses (N)") ///
		xtitle("Days since launch") ///
		title("Cumulative Responses by Treatment Arms") ///
		ylabel(, grid) xline(1 3 7 14 21, lpattern(dash) lcolor(gs12)) ///
		name(plot_treatment, replace)

	* 4. Simpan
	local date : display %tdDNCY daily("$S_DATE", "DMY")
	capture mkdir "$output/Agent Baseline - `date'"
	graph export "$output/Agent Baseline - `date'/2 - cumulative_response_plot.png", name(plot_treatment) as(png) replace
	...
	twoway `dayplots', legend(order(`legendorder') cols(3) pos(6) ring(1)) ytitle("Responses (N)") xtitle("Days since launch") title(	"Cumulative Responses by Treatment Arms") ylabel(, grid) xline(1 3 7 14 21, lpattern(dash) lcolor(gs12)) name(cum_plot, replace)

capture mkdir "$output/Agent Baseline - `date'"
graph export "$output/Agent Baseline - `date'/2 - cumulative_response_plot.png", name(cum_plot) as(png) replace
	...
	* 2) Export: unstack = jadikan tiap variabel sebagai kolom
	esttab using "$output/Agent Baseline - `date'/summary_overall_by_treatment.tex", ///
		cells("sum(fmt(%9.0f))") ///
		unstack ///
...
*--------------------------------------------------------------------------
	...
	destring q_3b q_3d q_3e q_4c prior posterior q_8b q_8c_1 q_9a, replace force
**# Summary table
	...
	lab var prior   "Prior: Estd pc change in agent number"
	lab var posterior    "Posterior: Estd pc change in agent number"
	...
	*** Sumstat (general)
	...
	eststo a: estpost summarize ///
			q_3b q_3d q_3e q_4c prior posterior q_8b q_8c_1 q_9a, detail
	esttab a using "$output/Agent Baseline - `date'/2 sumstat_overall.tex", replace ///
	...
	*** Sumstat (by treatment groups)
	...
	forval t = 1/5 {
		...
		eststo z: estpost summarize ///
				prior posterior q_7a if treatment_status == `num', det
...
preserve
	...
	* Export to LaTeX (requires estout)
	...
	esttab using "$output/Agent Baseline - `date'/variable_uniques.tex", ///
		cells("mean(fmt(%9.0f))") ///
		replace booktabs nonumber nomtitle noobs label ///
...
**# 2. Section 2 (WAIT FOR MARTIN'S FEEDBACK) -- ERROR
*** q_2a
	preserve
	...
	duplicates drop unique_code, force
...
*** prior (Prior Beliefs: Overall)
preserve

	destring prior, replace force
	** Drop missing variable (if any)
	drop if missing(prior)

	** Summary statistics
	qui summarize prior, detail
	return list
	...
	** Detect and drop outlier(s)
	...
	generate lower_limit = r(p5) - 1.5 * iqr
	generate upper_limit = r(p95) + 1.5 * iqr
	generate outlier = (prior < lower_limit) | (prior > upper_limit)
	drop if outlier == 1
	...
	* Histogram
	...
	histogram prior, percent color("255 158 128") ///
		discrete ///
		xlabel(-100(10)100) ///
		ylabel(0(2)10) ///
	...
	* Box plot
	...
	graph box prior, yline(`r(mean)', lpattern(.)) ///
		ytitle("In percentage (%)", size(medsmall)) ///
		title("Estimated change in the number of agents in %", size(medsmall)) ///
		subtitle("{bf: Prior beliefs}", size(medsmall)) ///
		text(`r(p50)' 95 "Median=`median_rounded'", size(vsmall)) ///
...
*** prior (Prior Beliefs: By Treatment Status)
* Histogram
	destring prior, replace force 
	
	forval a = 0/4 {
		gen gr_prior_t`a' = prior if treatment_status == `a'
	}

	label var gr_prior_t0 "{bf: Prior beliefs (Pure Control)}"
	forvalues i = 1/4 {
		label var gr_prior_t`i' "{bf: Prior beliefs (T`i')}"
	}

	local a = 0
	local a = 0
foreach b of varlist gr_prior_t0-gr_prior_t4 {
    preserve
    
    drop if missing(`b')
    qui su `b', det
    
    * 1. Identifikasi Outlier (Tanpa membuat variabel permanen agar tidak bentrok)
    local iqr = r(p95) - r(p5)
    local ll  = r(p5) - (1.5 * `iqr')
    local ul  = r(p95) + (1.5 * `iqr')
    
    count
    local total_before = r(N)
    drop if `b' < `ll' | `b' > `ul'
    
    local dropped_obs = `total_before' - r(N)
    local z : var lab `b'

    * 2. Histogram
    local date : display %tdDNCY daily("$S_DATE", "DMY")
    histogram `b', percent color("255 158 128") ///
        discrete xlabel(-100(15)100) ylabel(0(2)10) ///
        xtitle("Estimated change in agents (%)") ///
        title("Estimated change: `z'") ///
        note("Total N = `r(N)'" "Dropped outliers = `dropped_obs'")
        
    graph export "$output/Agent Baseline - `date'/15 - prior_hist_T`a'.png", as(png) replace
...
preserve
	...
	destring posterior, replace force
	** Drop missing variable (if any)
	drop if posterior == .

	** Summary statistics
	qui summarize posterior, detail
	return list
	...
	** Detect and drop outlier(s)
	...
	generate lower_limit = r(p5) - 1.5 * iqr
	generate upper_limit = r(p95) + 1.5 * iqr
	generate outlier = (posterior < lower_limit) | (posterior > upper_limit)
	drop if outlier == 1
	...
	* Histogram
	histogram posterior, percent color("255 158 128") ///
		discrete ///
		xlabel(-100(10)100) ///
		ylabel(0(3)12) ///
		xtitle("Estimated change in the number of agents (in %)", size(medsmall)) ///
	...
	graph export "$output/Agent Baseline - `date'/17 - posterior_hist.png", as(png) replace
	...
	* Box plot
	...
	graph box posterior, yline(`r(mean)', lpattern(.)) ///
		ytitle("In percentage (%)", size(medsmall)) ///
		title("Estimated change in the number of agents in %", size(medsmall)) ///
		subtitle("{bf: Posterior beliefs}", size(medsmall)) ///
		text(`r(p50)' 95 "Median=`median_rounded'", size(vsmall)) ///
	...
	graph export "$output/Agent Baseline - `date'/17 - posterior_boxplot.png", as(png) replace

restore

*** Posterior (Posterior Beliefs: By Treatment Status)
* Histogram
	destring posterior, replace force 
	
	forval a = 1(2)3 {
		gen gr_posterior_t`a' = posterior if treatment_status == `a'
	}
	
	label var gr_posterior_t1 "{bf: Posterior beliefs (T1)}"
	label var gr_posterior_t3 "{bf: Posterior beliefs (T3)}"

	local a = 1
	foreach b of varlist gr_posterior_t1 gr_posterior_t3 {
		preserve
		...
		** Store lower and upper threshold
		...
		graph export "$output/Agent Baseline - `date'/17 - q_posterior_hist_T`a'.png", as(png) replace
...
* Randomization options check
	...
	destring q_7a_do_0 q_7a_do_1, replace

	recode q_7a_do_0 q_7a_do_1 (2 = 0)
	...
	graph bar q_7a_do_0 q_7a_do_1, percentages /// percent is the default
		ytitle("In percentage (%)", size(medsmall) orientation(vertical)) ///
		ylabel(0(25)100) ///
		blabel(bar, pos(top) size(medsmall) format(%15.1fc)) ///
...
*** q_8a_1
	*** # q_8a_1
    ...
    destring q_8a_1_*, replace force

    qui count if q_8a_1_1 != .
    local total_n = r(N)

    local list_var ""
    forval i = 1/8 {
        capture drop q_8a_1_`i'_final
        
        gen q_8a_1_`i'_final = 0 if q_8a_1_1 != .
        replace q_8a_1_`i'_final = 100 if q_8a_1_`i' == 1
        
        local list_var `list_var' q_8a_1_`i'_final
    }

    graph bar (mean) `list_var', ///
        ytitle("Percentage of Agents (%)", size(medsmall)) ///
        ylabel(0(20)100) ///
        blabel(bar, format(%9.1f) size(small)) ///
        title("To whom do you offer {bf:additional benefits}?", size(medsmall)) ///
        legend(order(1 "Friends" 2 "Family" 3 "High-value" 4 "New" ///
                     5 "Long-time" 6 "Poorer" 7 "Local" 8 "Competitor") ///
               rows(2) pos(6) ring(1) size(vsmall)) ///
        note("Total agents = `total_n'", size(vsmall))
...
preserve
	...
	* Box plot
	...
	local q1     = round(`r(p25)', 0.1)
	local q3     = round(`r(p75)', 0.1)
	local n_obs  = r(N)

	graph box q_8a_2, yline(`mean', lpattern(dash)) ///
		ytitle("Number of customers") ///
		title("Customers offered additional benefits", size(medsmall)) ///
		text(`med' 102 "Median=`med'", size(vsmall)) /// Koordinat X kecil saja (102)
		text(`q3'  102 "Q3=`q3'", size(vsmall)) ///
		text(`q1'  102 "Q1=`q1'", size(vsmall)) ///
		text(`mean' 102 "Mean=`mean'", size(vsmall)) ///
		note("Total agents = `n_obs'")
		
	local date : display %tdDNCY daily("$S_DATE", "DMY")
	graph export "$output/Agent Baseline - `date'/20 - q_8a_2_boxplot.png", replace
restore
restore
...
*** q_10a_1
	*** # q_10a_1
    ...
    qui sum q_10a_1 if q_10a_1 > 1000

    histogram q_10a_1 if q_10a_1 > 1000, percent color("255 158 128") ///
        discrete ///
        ...
        note("Total agents (valid) = `: di %6.0fc `r(N)''", size(medsmall))
