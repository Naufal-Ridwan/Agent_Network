...
****#Table for 1a #*****
	...
	* Export table (latex)
	...
	esttab tab_1a using "$path/output/tab_1a.tex", replace ///
        style(tex) booktabs ///
...
****#Table for 1b #*****
        ...
        * Export table (latex)
        ...
        esttab tab_1b using "$path/output/tab_1b.tex", replace ///
        style(tex) booktabs ///
        cells(b(fmt(3) star) se(par fmt(3))) ///
        collabels(none) starlevels(* 0.10 ** 0.05 *** 0.01) ///
...
esttab tab_second_stage using "$path/output/tab_second_stage.tex", replace ///
style(tex) booktabs ///
cells(b(fmt(3) star) se(par fmt(3))) ///
collabels(none) starlevels(* 0.10 ** 0.05 *** 0.01) ///
...
****#Table for first stage #*****
        ...
        * Export table (latex)
        ...
        esttab tab_first_stage using "$path/output/tab_first_stage.tex", replace ///
        style(tex) booktabs ///
        cells(b(fmt(3) star) se(par fmt(3))) ///
        collabels(none) starlevels(* 0.10 ** 0.05 *** 0.01) ///
...
preserve
binscatter posterior_prior signal_prior if received_info==1, msymbol(circle) savedata("temp/binned_treatment") replace line(none) nquantiles(40)
clear
qui do "temp/binned_treatment"
save "temp/binned_treatment",replace
restore

append using "temp/binned_treatment", gen(binned_treatment)
...
preserve
binscatter posterior_prior signal_prior if received_info==0, msymbol(circle) savedata("temp/binned_control") replace line(none) nquantiles(40)
clear
qui do "temp/binned_control"
save "temp/binned_control",replace
restore

append using "temp/binned_control", gen(binned_control)
...
graph export "$output/belief_updating_binscatter_c&t.png", as(png) replace
